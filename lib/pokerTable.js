...
class PokerTable extends EventEmitter {
  ...
  nextPlayableSeat(fromIdx) {
    for (let step = 1; step <= SEATS; step++) {
      const next = (fromIdx + step + SEATS) % SEATS;
  ...
  beginPreflopBetting() {
    ...
    for (let step = 1; step <= SEATS; step++) {
      const next = (this.bbSeat + step) % SEATS;
  ...
  beginBettingRound() {
    ...
    for (let step = 1; step <= SEATS; step++) {
      const next = (this.dealerSeat + step) % SEATS;
      if (this.toAct.has(next)) {
        this.turnSeat = next;
        break;
      }
...
module.exports = { PokerTable, SEATS, BUY_IN, SMALL_BLIND, BIG_BLIND };
