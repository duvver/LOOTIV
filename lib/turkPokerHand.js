...
function evaluate5(cards, minRank) {
  ...
  if (groups[0].count === 4) return [7, groups[0].rank, groups[1].rank]; // Four of a kind
  if (isFlush) return [6, ...ranks]; // Flush
  if (groups[0].count === 3 && groups[1].count === 2) return [5, groups[0].rank, groups[1].rank]; // Full House
  if (isStraight) return [4, straightHigh]; // Straight
  if (groups[0].count === 3) return [3, groups[0].rank, ...groups.slice(1).map((g) => g.rank)]; // Three of a kind
  if (groups[0].count === 2 && groups[1].count === 2) {
    const pairRanks = [groups[0].rank, groups[1].rank].sort((a, b) => b - a);
    return [2, pairRanks[0], pairRanks[1], groups[2].rank]; // Two pairs
  }
  if (groups[0].count === 2) return [1, groups[0].rank, ...groups.slice(1).map((g) => g.rank)]; // One pair
