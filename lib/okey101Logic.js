...
function canProcessSeri(meld, tile, okeySpec, position = null) {
  if (meld.kind !== 'seri') return null;
  const effTile = getEffectiveTile(tile, okeySpec);
  if (isWild(effTile, okeySpec)) {
    if (position === 'start' && meld.lo > 1) return { lo: meld.lo - 1, hi: meld.hi, at: 'start' };
    if (position === 'end' && meld.hi < 13) return { lo: meld.lo, hi: meld.hi + 1, at: 'end' };
    
    // Fallback if no specific position requested or requested position is invalid
