...
const OPEN_MIN = 51; // seri acma alt siniri (standart Turk Okey 101 kurali)
...
class Okey101Table extends EventEmitter {
  constructor() {
    ...
    this.totalScores = new Array(4).fill(0);
    this.gameOver = false;
    this.pot = 0; // Kumulatif pot: her el basinda STAKE toplanir, oyun sonunda dagitilir
  }
  ...
  async sit(user, seatIndex) {
    ...
    // Async gap sonrasi tekrar kontrol (race condition onlemi)
    if (this.findSeatByUser(user.id) !== -1) return { ok: true, alreadySeated: true };
    if (this.seats[seatIndex]) return { error: 'Koltuk dolu.' };
  ...
  addBot(seatIndex = null) {
    if (this.stage !== 'waiting') return { error: 'Oyun basladi, bot eklenemez.' };
    let idx = seatIndex;
  ...
  async startHand() {
    ...
    for (const seat of this.seats) {
      ...
      seat.penalizedOpenThisTurn = false;
    }

    // LT bakiye kontrolü yap
    for (let i = 0; i < SEATS; i++) {
      const s = this.seats[i];
      if (s && s.userId > 0) {
        try {
          const user = await db.getUserById(s.userId);
          if (!user || user.lt < STAKE) {
            this.log(`${s.name} bakiyesi yetersiz oldugu icin kaldirildi.`);
            this.removePlayer(s.userId);
          }
        } catch (err) {
          console.error('Bakiye kontrol hatasi:', err);
        }
      }
    }
    ...
    // Her el basinda STAKE topla (per-hand pot: stand-up exploit onlemi)
    for (let i = 0; i < SEATS; i++) {
      const s = this.seats[i];
      if (s && s.userId > 0) {
        this.pot += STAKE;
        db.adjustLt(s.userId, -STAKE).catch(err => console.error('Stake kesimi hatasi:', err));
      } else if (s && s.isBot) {
        this.pot += STAKE; // Botlarin potu sanal olarak eklenir
      }
    }
  ...
  handleOpen(userId, kind, groups) {
    ...
    if (kind === 'seri') {
      ...
      if (total < this.currentMinScore) {
        if (!seat.penalizedOpenThisTurn) {
          seat.penalizedOpenThisTurn = true;
          seat.extraPenalty += 101;
  ...
  handleSwapJoker(userId, meldId, tileId, targetJokerId = null) {
    const idx = this.findSeatByUser(userId);
    if (idx === -1) return { error: 'Masada degilsiniz.' };
    if (this.stage !== 'playing') return { error: 'El oynamiyor.' };
    if (this.turnSeat !== idx) return { error: 'Sira sizde degil.' };
    ...
    const seat = this.seats[idx];
    if (!seat.hasOpened) return { error: 'Okey boşa çıkarmak için önce elinizi açmalısınız.' };
    ...
    if (logic.isWild(tile, this.okeySpec)) {
      return { error: 'Okeyin yerine baska bir okey koyamazsiniz.' };
    }

    const meld = this.boardMelds.find(m => m.id === Number(meldId));
    if (!meld) return { error: 'Grup bulunamadi.' };

    let jokerIdx = -1;
    if (targetJokerId) {
      jokerIdx = meld.tiles.findIndex(t => t.id === Number(targetJokerId));
      if (jokerIdx !== -1 && !logic.isWild(meld.tiles[jokerIdx], this.okeySpec)) {
        jokerIdx = -1;
      }
    } else {
      jokerIdx = meld.tiles.findIndex(t => logic.isWild(t, this.okeySpec));
    }
    if (jokerIdx === -1) return { error: 'Bu grupta okey yok veya hedeflenen tas okey degil.' };

    // Kopya üzerinde test et
    const testTiles = [...meld.tiles];
    const extractedJoker = testTiles[jokerIdx];
    testTiles[jokerIdx] = tile;

    let isValid = false;
    if (meld.kind === 'seri' || meld.kind === 'per') {
      const v = logic.validateSeriOrPer(testTiles, this.okeySpec);
      if (v && v.kind === meld.kind) {
        if (meld.kind === 'seri' && v.lo === meld.lo && v.hi === meld.hi && v.color === meld.color) isValid = true;
        if (meld.kind === 'per' && v.number === meld.number) {
          isValid = true;
          meld.colorsUsed = v.colorsUsed;
        }
      }
    } else if (meld.kind === 'cift') {
      const v = logic.validateCift(testTiles, this.okeySpec);
      if (v) isValid = true;
    }

    if (!isValid) return { error: 'Bu tas okeyin yerine gecemez.' };

    // Degisimi uygula
    seat.tiles.splice(tIdx, 1);
    seat.tiles.push(extractedJoker);
    meld.tiles[jokerIdx] = tile;

    this.log(`${seat.name} okeyi bosa cikardi.`);
    this.emitUpdate();
    this.emitPrivateTiles();
  ...
  handleProcess({ userId, tileId, meldId, targetJokerId, position = null }) {
    const idx = this.findSeatByUser(userId);
    if (idx === -1) return { error: 'Masada degilsiniz.' };
    if (this.stage !== 'playing') return { error: 'El oynamiyor.' };
    if (this.turnSeat !== idx) return { error: 'Sira sizde degil.' };
    ...
    const currentProcessCount = this.processCounts.get(meld.id) || 0;
    if (meld.kind === 'per' && currentProcessCount >= 2) {
      return { error: 'Bir elde ayni pere en fazla 2 tas isleyebilirsiniz.' };
    }
    ...
    if (meld.kind === 'seri') {
      const fit = logic.canProcessSeri(meld, tile, this.okeySpec, position);
      ...
      if (!logic.isWild(tile, this.okeySpec)) {
        const effColor = (tile.joker && this.okeySpec) ? this.okeySpec.color : tile.color;
        meld.colorsUsed.push(effColor);
      }
    }
    ...
    this.processCounts.set(meld.id, currentProcessCount + 1);
  ...
  handleDiscard(userId, tileId) {
    ...
    if (isOkey && !isFinishing) {
      ...
      if (isProcessable && !isFinishing) {
        seat.extraPenalty += 101;
        this.log(`${seat.name} yere islek tas atti! +101 ceza.`);
      }
    }
  ...
  async settleCommon(rows, reason, finisherIdx = null) {
    ...
    // Update total scores
    for (const r of rows) {
      if (this.seats[r.seatIdx]) {
        this.seats[r.seatIdx].totalScore += r.penalty;
        this.totalScores[r.seatIdx] = this.seats[r.seatIdx].totalScore;
      }
    }
    ...
    for (const r of rows) {
      participants.push({ 
        ...
        penalty: r.penalty, 
        opened: r.opened,
        totalScore: this.seats[r.seatIdx] ? this.seats[r.seatIdx].totalScore : 0
      });
    }
    ...
    this.log(`El bitti (${reason}).`);

    this.gameOver = this.totalScores.some(score => score >= 101);

    if (this.gameOver) {
      this.emit('hand-result', { handNumber: this.handNumber, participants, gameOver: true });
      await this.endGame();
    } else {
      this.emit('hand-result', { handNumber: this.handNumber, participants, gameOver: false });
      this.finishHand();
    }
  }
  ...
  async endGame() {
    ...
    for (let i = 0; i < SEATS; i++) {
      if (this.seats[i]) {
        activeSeats.push(this.seats[i]);
        if (this.seats[i].totalScore < minScore) {
          minScore = this.seats[i].totalScore;
        }
      }
    }
    
    const winners = activeSeats.filter(s => s.totalScore === minScore);
    const totalPot = this.pot; // Oynanan tum ellerden biriken pot
    ...
    for (const seat of activeSeats) {
      let ltChange = 0; // STAKE zaten her el basinda kesildi
      if (seat.totalScore === minScore) {
        let winAmount = share + (remainder > 0 ? 1 : 0);
        if (remainder > 0) remainder--;
        ltChange = winAmount;
      }
      ...
      if (seat.userId > 0 && ltChange > 0) {
        await db.adjustLt(seat.userId, ltChange);
      }
    }
    
    const winnersStr = gameResult
      .filter(r => r.ltChange > 0)
      .map(r => `${r.name} (+${r.ltChange} LT)`)
      .join(', ');
      
    this.log(`Oyun bitti! Kazanan(lar): ${winnersStr}`);
    ...
    if (multiplier > 1) {
      for (const r of rows) {
        if (r.seatIdx !== finisherIdx) {
          // Carpan sadece tas cezasina uygulanir; kural ihlali cezalari (extraPenalty) oldugu gibi kalir
          r.penalty = (r.tilePenalty * multiplier) + r.extraPenalty;
        }
      }
    }
  ...
  handleTimeout() {
    ...
    // Yerden tas cekmis ve acma zorunlulugu varsa: zorunlulugu kaldir, ceza ver
    if (seat.mustOpenThisTurn) {
      seat.mustOpenThisTurn = false;
      seat.extraPenalty += 101;
      if (this.lastDrawnDiscard) {
        const tileId = this.lastDrawnDiscard.tile.id;
        seat.tiles = seat.tiles.filter(t => t.id !== tileId);
        this.discardPiles[this.lastDrawnDiscard.fromSeat].push(this.lastDrawnDiscard.tile);
        this.lastDiscard = { 
          tile: this.lastDrawnDiscard.tile, 
          fromSeat: this.lastDrawnDiscard.fromSeat, 
          availableToSeat: this.lastDrawnDiscard.availableToSeat 
        };
        this.lastDrawnDiscard = null;
        this.hasDrawn = false;
        this.log(`${seat.name} suresi doldu, acma yapamadi. Yandan aldigi tas geri kondu, desteden cekecek. +101 ceza.`);
        ...
        this.lastDrawnDiscard = null;
        this.log(`${seat.name} suresi doldu, acma yapamadi. +101 ceza.`);
      }
    }
