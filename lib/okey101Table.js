...
class Okey101Table extends EventEmitter {
  ...
  handleSwapJoker(userId, meldId, tileId) {
    const idx = this.findSeatByUser(userId);
    if (idx === -1) return { error: 'Masada degilsiniz.' };
    if (this.stage !== 'playing') return { error: 'El oynamiyor.' };
    if (this.turnSeat !== idx) return { error: 'Sira sizde degil.' };
    ...
    const jokerIdx = meld.tiles.findIndex(t => logic.isWild(t, this.okeySpec));
    if (jokerIdx === -1) return { error: 'Bu grupta okey yok.' };
    ...
    if (meld.kind === 'seri' || meld.kind === 'per') {
      ...
      if (v && v.kind === meld.kind) {
        ...
        if (meld.kind === 'per' && v.number === meld.number) isValid = true;
      }
  ...
  handleProcess({ userId, tileId, meldId, targetJokerId }) {
    ...
    if (currentProcessCount >= 2) {
      return { error: 'Bir sirada ayni pere en fazla 2 tas isleyebilirsiniz.' };
    }
