...
class Okey101Table extends EventEmitter {
  ...
  async endGame() {
    ...
    const totalPot = STAKE * playerCount * this.handNumber;
    ...
    for (const seat of activeSeats) {
      let ltChange = -(STAKE * this.handNumber);
      ...
      if (seat.userId > 0 && ltChange !== 0) {
        let actualNet = ltChange;
        if (ltChange < 0) {
          const dbUser = await db.getUserById(seat.userId);
          if (dbUser && dbUser.lt_balance < Math.abs(ltChange)) {
            actualNet = -dbUser.lt_balance;
          }
        }
        await db.adjustLt(seat.userId, actualNet);
      }
    }
